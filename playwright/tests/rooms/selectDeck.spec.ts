...
test('ルームページで、デッキを変更するとき、カードがオープン状態でも場がリセットされること', async ({ page }) => {
  await userJoinRoom(page, urls.room())
...
test('ルームページで、「Fibonacci」を選択したとき、フィボナッチ数列のカードが並ぶこと', async ({ page }) => {
  await userJoinRoom(page, urls.room())
  await page.selectOption('data-testid=deckSelect', 'sequential')
...
test('ルームページで、「Sequential」を選択したとき、1-10の数列のカードが並ぶこと', async ({ page }) => {
  await userJoinRoom(page, urls.room())
  await page.selectOption('data-testid=deckSelect', 'sequential')
...
test('ルームページで、「T-shirt size」を選択したとき、1-10の数列のカードが並ぶこと', async ({ page }) => {
  await userJoinRoom(page, urls.room())
...
test('ルームページで、別のメンバーがデッキを選択したとき、自分の手札に反映されること', async ({ page, browser }) => {
