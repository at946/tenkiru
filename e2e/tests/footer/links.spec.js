describe('footer/links', () => {
  ...
  test('トップページで、フッターのお問い合わせを選択したとき、開発者のTwitterプロフィールページへ遷移すること', async () => {
    await page.goto(urls.top);
    expect(await page.$eval('[data-testid="inquiry"]', (el) => el.innerText)).toBe('@asato');
    expect(await page.$eval('[data-testid="inquiry"]', (el) => el.getAttribute('href'))).toBe(
      'https://twitter.com/at_946',
    );
    expect(await page.$eval('[data-testid="inquiry"]', (el) => el.getAttribute('target'))).toBe(
      '_blank',
    );
  });
});
