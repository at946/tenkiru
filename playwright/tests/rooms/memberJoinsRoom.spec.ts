...
test('ルームページで、別の部屋に別のメンバーが入ってきたとき、カード置き場は増えないこと', async ({ page, browser }) => {
  const roomURL = urls.room()
  await page.goto(roomURL)
  await expect(page.locator('data-testid=tableCard')).toHaveCount(1)
...
test('ルームページで、カードがオープンした部屋に入室したとき、カードがオープンな状態からスタートすること', async ({ page, browser }) => {
  const roomURL = urls.room()
  await page.goto(roomURL)
  await expect(page.locator('data-testid=tableCard')).toHaveCount(1)
...
test('ルームページで、デッキがFibonacci以外を選択された状態で入室したとき、そのデッキが手札に表示されること', async ({ page, browser }) => {
  const roomURL = urls.room()
  await page.goto(roomURL)
  await expect(page.locator('data-testid=tableCard')).toHaveCount(1)
