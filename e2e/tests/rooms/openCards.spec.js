describe('rooms/openCards', () => {
  ...
  test('ルームページで、誰かがカードを場に出している状態で、オープンボタンを選択したとき、場に出たカードがオープンすること', async () => {
    ...
    let tableCardsValue = await page.$$eval('[data-testid="tableCard"]', els => els.map( el => el.innerText ))
    ...
    tableCardsValue = await page.$$eval('[data-testid="tableCard"]', els => els.map( el => el.innerText ))
  ...
  test('ルームページで、誰かがカードを場にでしている状態で、他のメンバーがカードをオープンできること', async () => {
    ...
    let tableCardsValue = await page2.$$eval('[data-testid="tableCard"]', els => els.map(el => el.innerText))
    ...
    tableCardsValue = await page2.$$eval('[data-testid="tableCard"]', els => els.map(el => el.innerText))
